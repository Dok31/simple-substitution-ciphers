...
bar = IncrementalBar('Countdown', max=12*12*26*26)
LANG = 'en'
...
def bruteforse_for_af_rec(shifr_text):
    ...
    for i in range(len(a1_keys)):
        for j in range(len(a2_keys)):
            for b1 in range(M):
                for b2 in range(M):
                    posible_text = deshifr_recur_afin(shifr_text, (a1_keys[i], b1, a2_keys[j], b2))
                    letters_cou = 0
                    ...
                    coefs_of_frequencys_and_keys.append([coincidence_of_letters, (a1_keys[i], b1, a2_keys[j], b2)])
